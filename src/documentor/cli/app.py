...
def generate(
    ...
    _run_generation(docs_to_generate)
...
@app.command(help="Quickly add an autogenerated readme file to your repo")
def add_readme(
    ...
    global config, state_manager, parser, writer
    console.print("[blue]Adding autogenerated README.md...[/blue]")
    ...
    if not style_existed:
        ...
        with open(style_path, "w", encoding="utf-8") as f:
            f.write(content)
        console.print(f"[green]Created {style_path} using basic template.[/green]")
    ...
    _run_generation([doc_item])

    if once:
        console.print("[blue]Cleaning up documentor state files (--once)...[/blue]")
        if config_manager.config_exists():
            os.remove(config_manager.config_file)
        state_manager.clear_statefile()
        if not style_existed and os.path.exists(style_path):
            os.remove(style_path)
        console.print("[green]Cleanup complete! Left only the README.md[/green]")
...
def remove(
    ...
    console.print(f"[blue]Removing {target_file}...[/blue]")

    filepath = Path(target_file)
    is_tracked = any(ds.filepath == filepath for ds in state_manager.state.managed_docs)

    if not is_tracked:
        console.print(f"[yellow]Warning: {target_file} is not currently tracked by Documentor.[/yellow]")
    ...
    if os.path.exists(target_file):
        try:
            ...
            console.print(f"[green]Deleted file: {target_file}[/green]")
        except Exception as e:
            console.print(f"[red]Error deleting file {target_file}: {e}[/red]")

    if is_tracked:
        state_manager.remove_doc(filepath)
        console.print(f"[green]Untracked {target_file} from Documentor lock state.[/green]")

    console.print(f"[green]Successfully removed {target_file}.[/green]")
...
def destroy(
    ...
    console.print("[blue]Destroying Documentor configuration...[/blue]")
    ...
    if with_files:
        console.print("[blue]Deleting tracked documentation files...[/blue]")
        for ds in state_manager.state.managed_docs:
            if os.path.exists(ds.filepath):
                try:
                    os.remove(ds.filepath)
                    console.print(f"[green]Deleted {ds.filepath}[/green]")
                except Exception as e:
                    console.print(f"[red]Error deleting {ds.filepath}: {e}[/red]")

    if config_manager.config_exists():
        os.remove(config_manager.config_file)
        console.print("[green]Deleted documentor.yaml[/green]")
    else:
        console.print("[yellow]documentor.yaml not found.[/yellow]")

    if state_manager.statefile_exists():
        state_manager.clear_statefile()
        console.print("[green]Deleted documentor-lock.yaml[/green]")
    else:
        console.print("[yellow]documentor-lock.yaml not found.[/yellow]")

    console.print("[green]Documentor successfully destroyed for this project.[/green]")
