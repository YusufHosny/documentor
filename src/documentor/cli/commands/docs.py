...
from documentor.llm.chains import generate_plan, async_sync_docs
...
def generate(
    ...
    if force_regenerate:
        ...
        stale_docs = {str(Path(ds.filepath)) for ds in ctx.state_manager.get_stale_docs()}
        for doc in managed_docs_items:
            target_dir = os.path.dirname(doc.filename) or ""
            filepath = doc.filename
            if (
                not doc.filename.lower().endswith("readme.md")
                and not target_dir.lower().startswith(ctx.config.docs_dir.lower())
            ):
                filepath = os.path.join(ctx.config.docs_dir, doc.filename)
            filepath_str = str(Path(filepath))
            if not os.path.exists(filepath) or filepath_str in stale_docs:
...
def sync():
    ...
    use_agent = should_use_agent(ctx.config, ctx.parser)
    ...
    if docs_to_sync:
        if use_agent:
            ...
            context = ctx.parser.extract_context()
            final_paths = asyncio.run(
                async_sync_docs(context, ctx.config, ctx.state_manager, docs_to_sync)
            )
