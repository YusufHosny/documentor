...
class DocItem(BaseModel):
    ...
    description: str = Field(
        description="A brief description of the document's purpose or focus"
    )
...
class Config(BaseModel):
    provider: Literal["openai", "vertexai", "ollama"] = Field(
        default="vertexai", description="LLM provider"
    )
    model: str = Field(default="gemini-3.1-pro-preview", description="Model name")

    docs_dir: str = Field(
        default="docs", description="Output directory for generated documentation"
    )
