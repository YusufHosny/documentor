...
class StateManager:
    ...
    def get_current_hash(self, paths: Optional[List[str]] = None) -> str:
        ...
        if self.config.use_git:
            try:
                ...
                untracked_hash = (
                    self._hash_files(untracked_files) if untracked_files else ""
                )
                ...
                raise RuntimeError(
                    "Git is not available or this is not a git repository. Cannot compute project hash."
                )
    ...
    def upsert_doc(
        ...
        current_hash = self.get_current_hash(
            source_refs if tracking_type == "file" else None
        )
        ...
        if existing_doc:
            description = (
                existing_doc.description if description is None else description
            )
            tracking_type = (
                existing_doc.tracking_type if tracking_type is None else tracking_type
            )
            source_refs = (
                existing_doc.source_refs if source_refs is None else source_refs
            )
        else:
    ...
    def get_stale_docs(self) -> List[DocState]:
        ...
        for ds in self.state.managed_docs:
            current_hash = self.get_current_hash(
                ds.source_refs if ds.tracking_type == "file" else None
            )
