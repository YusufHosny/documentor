...
* **`use_style_md`** *(boolean, default: false)*: Enables the use of a custom style guide file for formatting instructions.
* **`style_md_path`** *(string, default: "docs/style.md")*: The file path to your custom style instructions if `use_style_md` is enabled.
...
* **`required_files`** *(list)*: A list of documentation files that must be generated and managed. Each item requires:
  * `filename`: The target markdown file name (e.g., `api.md`).
  * `description`: A brief summary of the document's purpose.
  * `type`: The category or format of the document (e.g., `Overview`, `API Reference`).
...
* **`use_agent`** *(boolean, default: false)*: Manually enables Agent Mode for dynamic, token-efficient context extraction.
* **`agent_threshold_kb`** *(integer, default: 1000)*: The codebase size threshold (in KB) above which Agent Mode is automatically triggered.
